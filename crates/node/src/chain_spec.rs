...
use app_chain_runtime::{AuraConfig, GenesisConfig, GrandpaConfig, SealingMode, SystemConfig, WASM_BINARY};
...
impl sp_runtime::BuildStorage for DevGenesisExt {
    fn assimilate_storage(&self, storage: &mut Storage) -> Result<(), String> {
        BasicExternalities::execute_with_storage(storage, || {
            app_chain_runtime::Sealing::set(&self.sealing);
        });
...
fn testnet_genesis(
    ...
    let starknet_genesis_config: app_chain_runtime::pallet_starknet::GenesisConfig<_> = genesis_loader.into();
